...
#     """
#     # Query finished games where our model participated. We order by game.started_at descending.
#     # Note: This example assumes two-player games.
...
#         )
#         if opponent_pg:
#             # If opponent is a model, use its name; otherwise (if human), use human_ip (if provided)
#             opponent = opponent_pg.model_name if opponent_pg.model_name else opponent_pg.human_ip or "Unknown"

#             # find the most recent elo before the game for the opponent

#             # find the change in elo for the model (i.e. most recent elo before and after the game of the the model)

#         else:
#             opponent = "N/A"
            
#         game_history.append({
#             "environment": game.specific_env_id,  # Or use a friendly name if available
#             "opponent": opponent,
#             "outcome": pg.outcome,  # outcome for your model in this game
#             "opponent_elo": 
...
#       - model_elo_change: the change in the model's elo based on its elo
#         before and immediately after the game.
#     """
#     import time
...
#         if opponent_pg:
#             # Determine the opponent identifier (model name or human_ip)
#             opponent = opponent_pg.model_name if opponent_pg.model_name else (opponent_pg.human_ip or "Unknown")
            
#             # --- 1. Find the most recent ELO for the opponent before the game ---
#             opponent_elo_record = (
#                 db.query(Elo)
#                 .filter(
...
#             # --- 2. Determine the model's ELO change ---
#             # Get model's ELO just before the game:
...
#                 .first()
#             )
#             model_elo_before = model_elo_before_record.elo if model_elo_before_record else DEFAULT_ELO #None

#             # Get model's ELO at or after the game's start (first record with updated_at >= game.started_at)
...
#                 .first()
#             )
#             model_elo_after = model_elo_after_record.elo if model_elo_after_record else None

#             if model_elo_before is not None and model_elo_after is not None:
#                 model_elo_change = round(model_elo_after - model_elo_before, 2)
#             else:
...
#         game_history.append({
#             "environment": game.specific_env_id,  # Use a friendly name if available
#             "opponent": opponent,
#             "outcome": pg.outcome,  # outcome for your model in this game
#             "opponent_elo": opponent_elo,
#             "model_elo_change": model_elo_change,
#             "started_at": time.strftime("%Y-%m-%d %H:%M", time.localtime(game.started_at)),
#         })
